(function ($) {
    ...
    $reply_button.on('click', function( event ) {
        ...
        //disabled submit if comment is blank
        ...
        $save_button.on('click', function ( event ) {
            ...
            var $self = $(this),
                ...
                reply_url = $SCRIPT_ROOT + '/post/' + post_id + '/' + comment_id+ '/reply';

            //send POST to reply to comment
            $.post(reply_url, { comment: $comment_text.val() } )
             .done( function () {
                 //get most recent comment to display
                 $.get( $SCRIPT_ROOT + '/comment/_newest', { post: post_id, author: "x"}, function ( data ) {
                     console.log(data);
                 });
             });
        });
